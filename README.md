...
東京メトロ×JR東日本の路線・駅・列車運行状況を3Dビューで可視化するWebアプリケーション（非公式PoC）。

🌐 **公開URL: https://railway.mirai-dx-platform.com** （Cloudflare Tunnel・現在はDEMOモック運行。ODPTトークン設定で実データ化）
...
### 🚑 障害復旧（公開URLが Cloudflare 1033 の場合）

`railway.mirai-dx-platform.com` が **Error 1033**（Tunnel がオリジンに到達不能）になる代表原因は、
**リポジトリを別ディレクトリへ移動した結果 systemd unit のパスが古いまま**になり、
アプリ（:3020）がクラッシュループして origin が消えるケース。復旧手順:
...
# 3. アプリ→Tunnel の順に復旧
systemctl --user daemon-reload
systemctl --user reset-failed metro3d.service
systemctl --user enable --now metro3d.service
...
| Phase 4 | ✅ 完了 | Cloudflare 公開 — https://railway.mirai-dx-platform.com （#5/#23） |
| Phase 5 | ✅ 完了 | UI 仕上げ（グラスHUD/運転席/実車風車両/JR5路線 #15-#24） |
| 保守 | 🔄 運用中 | 無期限保守フェーズ。残: ODPTトークン登録→実データ検証（#26・人間） |
